...
static any_t *get_xp_data(player_t *player, const char data[])
{
    any_t *stats = NULL;
    ...
    max_xp_data = dico_t_get_value(stats->value.dict, data);
    ...
    object_t *object)
{
    ...
    max_left = object->bigdata.sprite_bigdata.rect.width;
    object->bigdata.sprite_bigdata.rect.left = (max_left / max_xp) * actual_xp;
    return RET_OK;
}
    ...
    window_t *window, float time)
{
    ...
    if (is_xp_changed(actual_xp) || is_xp_max_upgraded(max_xp)) {
        change_xp_bar_stats(max_xp, actual_xp, object);
    }
}
